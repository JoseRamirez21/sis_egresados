...
if (!empty($_POST["btnregistrarCer"])) {
   if (!empty($_POST["txtestudiante"]) and !empty($_POST["txtmodulo"])) {
      ...
      $verificarNombre = $conexion->query(" select count(*) as 'total' from constancia where id_estudiante=$estudiante and id_modulo=$modulo and anio='$anio' ");
      ...
      if ($total > 0) { ?>
         ...
         try {
            $registrar = $conexion->query(" insert into constancia(id_estudiante,id_modulo,anio,imagen) values($estudiante,$modulo,'$anio','$imagen') ");
         } catch (\Throwable $th) {
            $registrar = 0;
         }
         if ($registrar == true) {
